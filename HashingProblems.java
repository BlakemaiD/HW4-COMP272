...
public class HashingProblems {
        ...
        public double getAverage(HashMap<Integer, Integer> map, int[] array) {
                ...
                for (int i = 0; i < array.length; i++) {
                        ...
                        int val = array[i];  //current val from the array
                        if (map.containsKey(val)) { //check in hashmap
                                sum += map.get(val);
                                total++;
                        }
                }
        ...
        public ArrayList<String> odd(HashMap<Integer, String> map) {
                ...
                Iterator<Integer> itr = map.keySet().iterator(); //iterates through the hashmap
                while (itr.hasNext()) {
                        int key = itr.next();
                        if (key % 2 != 0) { //checks odd
        ...
        public int twoSums(int[] numbers, int k) {
                ...
                for (int i = 0; i < numbers.length; i++) { //add the array variables to HashSet
                        set.add(numbers[i]);
                }
                for (int i = 0; i < numbers.length; i++) { //keep loop separate to make sure its O(n) and not O(n^2)
                        /*int compliment = i + k;
                        if (set.contains(compliment) && set.contains(i)) {
                                count++;
                        }*/
